...
emw_columns = ['IdNum', 'Word', 'Cob', 'IdNumLemma', 'FlectType', 'TransInfl']
...
esl_data = {
	item[0]: reduce_to_single_pos(dict(zip(esl_columns, item)))
	for item in esl_data
}
...
eml_data = {
	item[0]: add_missing_segmentation(dict(zip(eml_columns, item)) | {'pos': esl_data[item[0]]['pos']})
	for item in eml_data
}
json_write(eml_data, DATA / 'celex_eml.json')


emw_data_path = CELEX / 'english' / 'emw' / 'emw.cd'
with open(emw_data_path) as file:
	emw_data = [line.strip().split('\\') for line in file]
emw_data_dict = defaultdict(list)
for item in emw_data:
	emw_data_dict[item[1]].append( dict(zip(emw_columns, item)) | {'pos': esl_data[item[3]]['pos']} )
json_write(emw_data_dict, DATA / 'celex_emw.json')
